    ...
    private InsuranceData getInsuranceData(InsuranceWorkerType type, Long patientId) {
        ...
        switch (type) {
            case Commercial:
                ...
                commercialRepository.findByPatient_Id(patientId);
                data = InsuranceDataMapper.map(commercialRepository.findByPatient_Id(patientId));
                break;
            case Comp_NoFault:
                InsuranceWorkerInsuranceWorkerCompNoFaultRepository compNoFaultRepository = BeanFactory.getBean(InsuranceWorkerInsuranceWorkerCompNoFaultRepository.class);
                compNoFaultRepository.findByPatient_Id(patientId);
                break;
        }
        return data;
    }
